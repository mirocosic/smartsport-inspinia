...
Ext.define('Ext.ux.grid.feature.Searching', {
     ...
     */
    searchTipText: 'Unesite tekst za pretraživanje i pritisnite tipku Enter',
    /**
     ...
     */
     /**
     * @cfg {Array} extraIndexes Array of index names and dataIndex names to add to menu, 
     * e.g. [{header:'Person Title', dataIndex:'persTitle'}, {header:'Alternative Title',dataIndex:'persTitle2'}]
    ...
    onRender: function() {
        ...
        this.field = Ext.create('Ext.form.field.Trigger', {
            cls: "grid-search",
            width: this.width,
            selectOnFocus: (typeof this.selectOnFocus === 'undefined') ? true : this.selectOnFocus,
        ...
        if (true === this.autoFocus) {
            this.grid.getStore().on({scope: this, load: function() {
                    this.field.focus();
                }});
        }
...
//            }
            // fix for new 'lastOption' removal
            ...
            // add fields and query to extraParams of store
    ...
    , reconfigure: function() {
        ...
        if(this.extraIndexes instanceof Array){
            Ext.each(this.extraIndexes,function(config){
                var disable = false;
                var forceRadio=true;
                if(config.header && config.dataIndex) {
                    ...
                    Ext.each(this.forceRadioIndexes, function(item) {
                        forceRadio = forceRadio ? forceRadio : item === config.dataIndex;
                    });
                    if(!disable) {
                        menu.add('-')
                        menu.add(new Ext.menu.CheckItem({
                            ...
                            hideOnClick:false,
                            group:group,
                            checked:('all' === this.checkIndexes) && !forceRadio,
                            dataIndex:config.dataIndex,
                            forceRadio:forceRadio,
                            listeners:{
                                checkchange:function(it,checked){
                                    if(checked){
                                        if(forceRadio){
                                            it.parentMenu.items.each(function(i) {
                                                if(it !== i && i.setChecked && !i.disabled) {
                                                    i.setChecked(false);
                                                }
                                            });
                                        }else{
                                            it.parentMenu.items.each(function(i) {
                                                if(it !== i && i.setChecked && !i.disabled && i.forceRadio) {
                                                    i.setChecked(false);
                                                }
                                            });
                                        }   
