...
Ext.define('Ext.ux.form.field.DateTime', {
    ...
    /**
    * set combine errors to true for showing error
    */
    combineErrors: true,
    /**
    ...
    */
    initComponent: function() {
        ...
        me.dateField = new Ext.form.field.Date(Ext.apply({
            id:this.id + '-date',
            format:me.dateFormat || Ext.form.DateField.prototype.format,
            flex:1,
        ...
        me.timeField = new Ext.form.field.Time(Ext.apply({
            id:this.id + '-time',
            format:me.timeFormat || Ext.form.TimeField.prototype.format,
            flex:1,
            isFormField:false, //exclude from field query's
            submitValue:false,
            width: 100,
        ...
        me.add([me.dateField, me.timeField]);
        
        for (var i = 0; i < me.items.length; i++) {
            me.items.getAt(i).on('focus', Ext.bind(me.onItemFocus, me));
            me.items.getAt(i).on('blur', Ext.bind(me.onItemBlur, me));
            me.items.getAt(i).on('specialkey', function(field, event){
                key = event.getKey();
                tab = (key == event.TAB);
...
//        };
        ...
        //automatic current time on focus timefield added 2013-12-17
        me.timeField.on('focus',function(field){
            if(!field.getValue()){
