...
Ext.define('Ext.ux.desktop.TaskBar', {
    ...
    onQuickStartClick: function (btn) {
        ...
        if (module) {
            //changed framework window handling
            module.internalRun();
...
//            window.show();
    ...
    onWindowBtnResize: function (btn, width) {
        var win = btn.win;
        var txtLength = Ext.util.TextMetrics.measure(btn.btnInnerEl, win.title).width;

        var charsLength = Math.floor(win.title.length * (btn.btnInnerEl.getSize(true).width / txtLength));

        if(win.title.length > charsLength) {
            btn.updateQTip(win.title);
        } else {
           btn.updateQTip();
        }

        btn.setText(Ext.util.Format.ellipsis(win.title, charsLength));
    },
    ...
    addTaskButton: function(win) {
        var config = {
            ...
            listeners: {
                ...
                resize: this.onWindowBtnResize,
                boxReady: this.onWindowBtnResize,
                scope: this
            },
            win: win
        };
